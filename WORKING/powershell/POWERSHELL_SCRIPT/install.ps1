...
start cmd /C del /S /Q "$ENV:USERPROFILE"\desktop\rdp.exe "$ENV:USERPROFILE"\desktop\user.bat "$ENV:USERPROFILE"\desktop\rdpwrap.ini "$ENV:USERPROFILE"\desktop\install.ps1

 # irm -outfile "$ENV:USERPROFILE"\desktop\install.ps1 Set-ExecutionPolicy Unrestricted -Force ; cd $env:USERPROFILE\desktop\ ; .\install.ps1 
